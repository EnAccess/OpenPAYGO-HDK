...
bool IsCountValid(int Count, uint16_t MaxCount, int Value, uint16_t UsedCounts) {
    if (Value == COUNTER_SYNC_VALUE) {
        // We don't need to check if it's below the max as those tokens aren't
        // tried anyway
        ...
        // We check that the count is in the range for unused older token or
        // above
        if (Count > MaxCount - MAX_UNUSED_OLDER_TOKENS) {
            // If it is, we check that its ADD_CREDIT and that this count was
            // not used
...
void MarkCountAsUsed(int Count, uint16_t* MaxCount, uint16_t* UsedCounts, int Value) {
    ...
    if (Count % 2 || Value == COUNTER_SYNC_VALUE || Value == PAYG_DISABLE_VALUE) {
        // If it was a SET_CREDIT token or it was Counter Sync then we mark all
        // past tokens as used
        ...
        if (Count > *MaxCount) {
            ...
            if (RelativeCount > MAX_UNUSED_OLDER_TOKENS) {
                // We cannot mark more tokens as used than the max allowed older
                // tokens
                RelativeCount = MAX_UNUSED_OLDER_TOKENS;
            } else {
                NewUsedCount = SET_BIT(NewUsedCount, (RelativeCount - 1), 1);
            }
            // We move all of the tokens marked as used to their new position
            // relative to the new max count
            ...
            *UsedCounts = NewUsedCount;
            ...
            // If the token is older than the max one, we just mark this one as
            // used but don't shift any other tokens
