...
void setupNFC() {
    ...
    if (!versiondata) {
        ...
        while (1); // We don't start if there's no module
    }
    // Print the module info
    debugPrint("INFO: Found PN53x NFC module"); debugPrintln((versiondata>>24) & 0xFF, HEX);
    debugPrint("INFO: NFC Firmware ver. "); debugPrint((versiondata>>16) & 0xFF, DEC);
    debugPrint('.'); debugPrintln((versiondata>>8) & 0xFF, DEC);
}
...
void checkNFCTapped() {
  if (readerDisabled) {
    ...
    handleCardDetected();
    // The section below can be enabled instead of the line above if your NFC module supports it
...
void handleCardDetected() {
  ...
  if (success) {
    ...
    debugPrint(" | UID Length: ");debugPrintln(uidLength, DEC);
    success = readCard();
    if(success) {
      ...
      if(success) {
        ...
        // 1 blue + 1 red blink means card could be read but no valid data
        blinkBlueLED(1);
        blinkRedLED(1);
      }
    } else {
...
bool generateData() {
  ...
  if(serializedSize < maxDataSize) {
    serializeJson(outgoingJSON, wholeCardBuffer+TAG_HEADER_SIZE+MEDIA_TYPE_SIZE, MAX_TAG_SIZE-TAG_HEADER_SIZE-MEDIA_TYPE_SIZE); // We serialize into the card buffer
